...
<style>
    ...
    .history-section h3 button:hover { background: #c82333; }
    .history-list {
        max-height: 300px;
        overflow-y: auto;
    }
    ...
    .history-item:hover {
        border-color: #4a90d9;
        box-shadow: 0 1px 3px rgba(74, 144, 217, 0.2);
    }
    .history-item-time {
        font-size: 11px;
        color: #888;
...
function getGraphPreview(graph) {
    // Extract a meaningful preview from the graph
...
function updateHistorySelection(id) {
    // Remove previous selection
    const prev = document.querySelector('.history-item.selected');
...
function loadHistoryEntry(id) {
    ...
    document.querySelector('textarea[name="graph"]').value = entry.graph;
    document.querySelector('textarea[name="path"]').value = entry.path;

    // Mark that we're restoring from history (don't add duplicate)
...
document.getElementById('history-list').addEventListener('click', function(e) {
    ...
    if (item) {
        loadHistoryEntry(parseInt(item.dataset.id, 10));
    }
});

// Clear selection when user edits the textareas
document.querySelector('textarea[name="graph"]').addEventListener('input', clearHistorySelection);
document.querySelector('textarea[name="path"]').addEventListener('input', clearHistorySelection);
